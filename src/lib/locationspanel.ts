...
export function setupLocationsPanel(
  ...
  scrollbar.update();
  renderer.shutdownSignal.addEventListener("abort", () => scrollbar.destroy(), {
    once: true,
  });
  ...
  input.addEventListener(
    ...
    () => {
      ...
      results.replaceChildren(
        ...parseLocationTargets(query).map((target) =>
          createResult(target, "GO"),
        ),
      );
      scrollbar.update();
      status.textContent = "";
  ...
  renderer.shutdownSignal.addEventListener("abort", clearSearch, {
    once: true,
  });
  sync();
}

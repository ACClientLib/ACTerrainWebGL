...
const EMPTY_DIAGNOSTICS: CacheDiagnostics = { enabled: false, usageBytes: 0, quotaBytes: 0, cacheBytes: 0 }
...
class OpfsCacheWorkerClient {
  ...
  async getMany(namespace: CacheNamespace, keys: string[]): Promise<(CachedResource | null)[]> {
    if (keys.length === 0) return []
    if (!await this.ready || !this.worker) return keys.map(() => null)
    const values = await this.request({ operation: 'getMany', namespace, keys, queuedAt: Date.now() })
    return values ?? keys.map(() => null)
  }

  setMany(namespace: CacheNamespace, entries: readonly (readonly [string, CachedResource])[]): Promise<void> {
    if (entries.length === 0) return Promise.resolve()
    void this.ready.then(enabled => {
      if (!enabled || !this.worker) return
      const request: CacheWorkerRequest = { operation: 'setMany', namespace, entries }
      this.worker.postMessage(request)
    })
    return Promise.resolve()
  }

  async removeOtherVersions(namespace: CacheNamespace, formatVersion: number, datasetVersion: string): Promise<void> {
    if (!await this.ready || !this.worker) return
    await this.request({ operation: 'removeOtherVersions', namespace, formatVersion, datasetVersion, queuedAt: Date.now() })
  }
  ...
  private request(request: AwaitedCacheRequest): Promise<(CachedResource | null)[] | undefined> {
  ...
  private handleMessage(message: CacheWorkerMessage): void {
    if (message.type === 'ready') {
      ...
      if (!this.initialized) {
        ...
        if ('requestIdleCallback' in globalThis) requestIdleCallback(() => this.deleteIndexedDbCaches(), { timeout: 5000 })
  ...
  private disable(reason: string): void {
    ...
    if (!this.initialized) {
      ...
      console.info(`OPFS resource cache disabled; using network-only loading: ${reason}`)
    }
...
export class DatObjectCache {
  ...
  get loadTimings(): LoadingTimingSnapshot { return sharedWorker.loadTimings }
  get diagnostics(): CacheDiagnostics { return { ...sharedWorker.diagnostics } }
  ...
  setMany(entries: readonly (readonly [string, CachedResource])[]): Promise<void> {
    return sharedWorker.setMany(this.namespace, entries)
  }

  removeOtherVersions(formatVersion: number, datasetVersion: string): Promise<void> {
    return sharedWorker.removeOtherVersions(this.namespace, formatVersion, datasetVersion)
  }

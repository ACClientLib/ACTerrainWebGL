...
export class TerrainRenderer {
  ...
  constructor(
    ...
    this.#gpuTimerExtension = this.gl.getExtension(
  ...
  #addSettings() {
    ...
    gui
      .add(settings.data, settings.labels.showServerSpawns)
      .name("Show Server Spawns");
    gui
      .add(settings.data, settings.labels.showParticles)
      .name("Show Particles");
    gui
      .add(settings.data, settings.labels.sceneryEnabled)
      .name("Enable Scenery")
      .onChange((enabled: boolean) => {
        if (!enabled) this.#sceneryRenderer.clear();
        else void this.#loadScenery();
      });
    gui
      .add(settings.data, settings.labels.minZoomFor3DObjects, 0.05, 5, 0.05)
      .name("3D Object Zoom");
    gui
      .add(
        {
          clearDatCache: () =>
            this.#objects
              ...
              .catch((error) =>
                console.error(
                  "Unable to clear ACTerrain resource cache",
                  error,
                ),
              ),
        },
    ...
    flyingControls
      .add(this.#objects, "loadDistance", 1, 32, 1)
      .name("3D Object Load Distance");
    this.#moveSpeedController = flyingControls
      .add(this.flyingCamera, "MoveSpeed", 0.1, 400)
      .name("Move Speed");
  ...
  switchCamera(mode: CameraMode) {
    ...
    const flyingMapPosition =
      oldMode === CameraMode.Flying ? this.flyingCamera.GetMapPosition() : null;
    ...
    if (mode === CameraMode.Camera2D) {
      ...
      if (oldMode === CameraMode.Flying) {
        ...
        const terrainHeight = this.getTerrainHeightAt(
          this.flyingCamera.Position.x,
          this.flyingCamera.Position.y,
        );
  ...
  private flyingHeightForViewport(targetWidth: number, targetHeight: number) {
    ...
    const unitWidth = Math.max(
      0.000001,
      Math.abs(unitFootprint[1].x - unitFootprint[0].x),
    );
    const unitHeight = Math.max(
      0.000001,
      Math.abs(unitFootprint[2].y - unitFootprint[0].y),
    );
    return Math.max(
      1,
      referenceHeight *
        Math.max(targetWidth / unitWidth, targetHeight / unitHeight),
    );
  }
  ...
  getTerrainHeightAt(worldX: number, worldY: number) {
    if (!this.#terrainHeightData && this.#dataTexture.image.complete) {
      ...
      canvas.height =
        this.#dataTexture.image.naturalHeight || TERRAIN_DATA_SIDE;
      const context = canvas.getContext("2d");
      if (context) {
        context.drawImage(this.#dataTexture.image, 0, 0);
        this.#terrainHeightData = context.getImageData(
          0,
          0,
          canvas.width,
          canvas.height,
  ...
  #setupGL() {
    ...
    this.#showLandcellLinesLoc = this.gl.getUniformLocation(
      this.program!,
      "showLandcellLines",
    );
    this.#showLandblockLinesLoc = this.gl.getUniformLocation(
      this.program!,
      "showLandblockLines",
    );
  ...
  #setConstantUniforms() {
    this.gl.uniform1fv(
      this.#heightTableLoc,
      new Float32Array(terrainHeightTable),
    );
    this.gl.uniform3fv(
      this.#terrainColorsLoc,
      new Float32Array(
        terrainColors.flatMap((color) => [color.x, color.y, color.z]),
      ),
    );
  ...
  #makeTextures() {
    ...
    this.#dataTexture = new Texture(
      ...
      new Vector2(TERRAIN_DATA_SIDE, TERRAIN_DATA_SIDE),
      0,
    );
  ...
  #setUniforms() {
    ...
    this.gl.uniform1f(
      this.#minZoomForTexturesLoc!,
      settings.data.minZoomForTextures,
    );
    this.gl.uniform1f(
      this.#showLandcellLinesLoc,
      settings.data.showLandcellLines ? 1.0 : 0.0,
    );
    this.gl.uniform1f(
      this.#showLandblockLinesLoc,
      settings.data.showLandblockLines ? 1.0 : 0.0,
    );
    if (this.currentCameraMode === CameraMode.Camera2D) {
      ...
      const minX = Math.max(
        0,
        Math.floor(Math.min(topLeft.x, bottomRight.x) / LAND_BLOCK_SIZE) - 1,
      );
      const visibleMinY = Math.min(topLeft.y, bottomRight.y);
      const visibleMaxY = Math.max(topLeft.y, bottomRight.y);
      const minY = Math.max(
        0,
        Math.floor((camera2D.MapSize.y - visibleMaxY) / LAND_BLOCK_SIZE) - 1,
      );
      const maxX = Math.min(
        LAND_BLOCK_SIDE,
      ...
      const visibleBlocks = this.#visibleTerrain3D(
        this.currentCamera as CameraFlying,
      );
  ...
  #visibleTerrain3D(camera: CameraFlying): [number, number][] {
    ...
    const points = [
      ...
      ...samples.map(([x, y]) => {
        ...
        const distance =
          ray.direction.z < -0.000001
            ? Math.max(0, (0 - ray.origin.z) / ray.direction.z)
            : mapExtent;
        return ray.origin
          .clone()
          .add(ray.direction.clone().scale(Math.min(distance, mapExtent)));
      }),
    ];
    const minX = Math.max(
      0,
  ...
  #updateOverlay() {
    ...
    const fetching =
      this.#objects.pendingApiRequestCount +
      this.#sceneryRenderer.frameDiagnostics.pendingRequests;
    const fetchingInfo = settings.data.showDebug
      ...
      : `Fetching: ${fetching}`;
    const objectLoadInfo =
      this.#objects.objectLoadState === "error"
    ...
    if (this.currentCameraMode === CameraMode.Camera2D) {
      const camera2D = this.currentCamera as Camera2D;
      coordsInfo = formatMapCoordinates(
      ...
      coordsInfo = formatMapCoordinates(
        worldToMapCoordinates(flyingCam.Position),
      );
    }
  ...
  private formatLoadingTimings(
    label: string,
    timings: LoadingTimingSnapshot,
  ): string {
    const entries = Object.entries(timings);
    if (entries.length === 0) return `${label}: no samples yet`;
    return (
      `${label} avg / p95 / max ms: ` +
      entries
        .map(
          ([name, timing]) =>
            `${name} ${timing.average.toFixed(1)} / ${timing.p95.toFixed(1)} / ${timing.max.toFixed(1)} (${timing.count})`,
        )
        .join(" | ")
    );
  }
  ...
  draw(dt: number) {
    ...
    this.#objects.render(
      ...
      settings.data.showObjects,
      settings.data.showServerSpawns,
      settings.data.showParticles,
      settings.data.minZoomFor3DObjects,
    );
    if (settings.data.sceneryEnabled)
      this.#sceneryRenderer.render(
        this.currentCamera,
        this.currentCameraMode,
        this.#objects.loadDistance * LAND_BLOCK_SIZE,
        settings.data.minZoomFor3DObjects,
      );
  ...
  #pollGpuTimer() {
    ...
    const disjoint = this.gl.getParameter(
      this.#gpuTimerExtension.GPU_DISJOINT_EXT,
    );
    ...
    if (available || disjoint) {
      if (available && !disjoint) {
        const gpuMs =
          Number(
            this.gl.getQueryParameter(this.#gpuQuery, this.gl.QUERY_RESULT),
          ) / 1_000_000;

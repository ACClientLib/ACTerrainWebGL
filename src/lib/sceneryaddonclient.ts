...
export class SceneryAddonClient {
  ...
  private resources = new Map<
    number,
    { kind: number; encoding: number; bytes: ArrayBuffer }
  >();
  ...
  constructor(
    private baseUrl = import.meta.env.VITE_ACTERRAIN_API_URL ??
  ...
  get state(): "disabled" | "loading" | "ready" | "error" {
    return this.loading
      ? "loading"
      : !this.descriptor
        ? this.error
  ...
  get visiblePlacementCount(): number {
    return [...this.loaded.values()].reduce(
      (sum, value) => sum + value.length,
      0,
    );
  }
  get indexedPlacementCount(): number {
    return [...this.chunks.values()].reduce(
      (sum, chunk) => sum + chunk.placementCount,
      0,
    );
  }
  get chunksById(): ReadonlyMap<number, SceneryChunkEntry> {
    return this.chunks;
  }
  get modelsByIndex(): readonly SceneryModel[] {
    return this.models;
  }
  modelIndex(value: number): number | undefined {
    return value < this.models.length
      ? value
      : this.modelIndexesByOriginalId.get(value);
  }
  get sceneryResourceBytes(): number {
    return [...this.resources.values()].reduce(
      (sum, resource) => sum + resource.bytes.byteLength,
      0,
    );
  }
  get dependenciesById(): readonly number[] {
    return this.dependencies;
  }
  resource(
    id: number,
  ): { kind: number; encoding: number; bytes: ArrayBuffer } | undefined {
  ...
  placementsForChunk(id: number): readonly SceneryPlacement[] {
    return this.loaded.get(id) ?? [];
  }
  get loadedChunkIds(): readonly number[] {
  ...
  async load(terrainVersion: string): Promise<void> {
    ...
    try {
      const descriptorResponse = await this.request(
        `/v1/scenery/${terrainVersion}/descriptor`,
      );
      if (!descriptorResponse.ok)
        throw new Error(
          `Scenery addon unavailable (${descriptorResponse.status})`,
        );
      this.descriptor = await descriptorResponse.json();
      if (
        this.descriptor.terrainDatasetVersion !== terrainVersion ||
        this.descriptor.formatVersion !== 1
      )
        throw new Error("Incompatible scenery addon");
      void this.cache
        .removeOtherVersions(
          SCENERY_FORMAT_VERSION,
          this.descriptor.addonVersion,
        )
        .catch(() => undefined);
      let source: ArrayBuffer;
      const cachedIndex = await this.profiler.measure("OPFS cache index", () =>
        this.cache.get(this.cacheKey("index")),
      );
      if (
        cachedIndex &&
        cachedIndex.formatVersion === SCENERY_FORMAT_VERSION &&
        cachedIndex.datasetVersion === this.descriptor.addonVersion
      ) {
        ...
        void this.cache
          .set(this.cacheKey("index"), this.cached(source, 0))
          .catch(() => undefined);
      }
      if (
        source.byteLength < 8 ||
        new DataView(source).getUint32(0, true) !== 0x49534341
      )
        throw new Error("Invalid ACTerrain scenery index");
      ...
      if (u32() !== 0x49534341 || u16() !== 1 || u16() !== 255)
        throw new Error("Invalid scenery index");
      u32();
      u32();
      u32();
      const count = u32();
      const legacyIndex = view.byteLength - offset === count * 40;
      const modelCount = legacyIndex ? 0 : u32();
      const resourceCount = legacyIndex ? 0 : u32();
      this.chunks.clear();
      ...
      if (legacyIndex || offset === source.byteLength) {
        this.models = [];
        this.dependencies = [];
        return;
      }
      this.models = Array.from({ length: modelCount }, () => ({
        ...
        dependencyStart: u32(),
        dependencyCount: u16(),
        eligibleForInstancing: view.getUint8(offset++) !== 0,
      ...
      if (this.models.some((model) => model.reserved !== 0))
        throw new Error("Invalid scenery model table");
      this.dependencies = Array.from(
        {
          length: this.models.reduce(
            (sum, model) => sum + model.dependencyCount,
            0,
          ),
        },
        u32,
      ...
      if (
        ...
        resourceCount <
          this.models.reduce(
            (max, model) => Math.max(max, model.meshResourceId),
            0,
          )
      )
        throw new Error("Invalid scenery resource table");
    } catch (error) {
  ...
  async loadCurrent(): Promise<void> {
    ...
    if (!response.ok)
      throw new Error(`Terrain dataset unavailable (${response.status})`);
    const descriptor = await response.json();
    if (typeof descriptor.version !== "string")
      throw new Error("Invalid terrain dataset descriptor");
    await this.load(descriptor.version);
  }

  async loadChunks(ids: number[], signal?: AbortSignal): Promise<void> {
    if (!this.descriptor) return;
    if (this.controller?.signal.aborted || !this.controller)
      this.controller = new AbortController();
    this.retainedChunks = new Set(ids);
    this.trim(ids);
    this.chunkQueue = this.chunkQueue.filter((id) =>
      this.retainedChunks.has(id),
    );
    ...
    const newChunks = ids.filter(
      (id) =>
        this.chunks.has(id) &&
        !this.loaded.has(id) &&
        !this.pending.has(id) &&
        !this.queuedChunks.has(id),
    );
    for (const id of newChunks.reverse()) {
      this.chunkQueue.unshift(id);
      this.queuedChunks.add(id);
    }
  ...
  private async runChunkScheduler(
    ...
    const worker = async () => {
      while (this.chunkQueue.length > 0 && !controller?.signal.aborted) {
        ...
        const promise = this.profiler.measure("chunk batch", () =>
          this.fetchChunks(batch, controller!.signal),
        );
        const chunkPromises = new Map<number, Promise<SceneryPlacement[]>>();
        for (const id of batch) {
          const chunkPromise = promise
            .then((chunks) => chunks.get(id)!)
            .catch(() => []);
        ...
        try {
          ...
          for (const [id, placements] of chunks)
            this.loaded.set(id, placements);
          this.trim([...this.retainedChunks]);
        } catch (error) {
          if (!controller?.signal.aborted)
            console.warn(`Scenery chunk batch skipped`, error);
        } finally {
          for (const id of batch)
            if (this.pending.get(id) === chunkPromises.get(id))
              this.pending.delete(id);
        }
      }
    };
    await Promise.all(
      Array.from({ length: SCENERY_REQUESTS_IN_FLIGHT }, worker),
    );
  }
  ...
  clear(): void {
    ...
    this.models = [];
    this.modelIndexesByOriginalId.clear();
    this.dependencies = [];
  ...
  async loadResources(ids: number[], signal?: AbortSignal): Promise<void> {
    ...
    const uncached = [...new Set(ids)].filter(
      (id) => !this.resources.has(id) && !this.pendingResources.has(id),
    );
    const existing = [...new Set(ids)].flatMap((id) => {
      const promise = this.pendingResources.get(id);
      return promise ? [promise] : [];
    });
    const batch =
      uncached.length === 0
        ? null
        : this.profiler.measure("resource load", () =>
            this.fetchResources(uncached, signal),
          );
    ...
    try {
      ...
      if (batch)
        for (const id of uncached)
          if (this.pendingResources.get(id) === batch)
  ...
  private async fetchResources(
    ...
    const cached = await this.cache
      .getMany(uncached.map((id) => this.cacheKey(`resource:${id}`)))
      .finally(() => {
        this.activeCacheReads--;
        this.profiler.record(
          "OPFS cache resources total",
    ...
    cached.forEach((entry, index) => {
      ...
      if (
        entry &&
        entry.formatVersion === SCENERY_FORMAT_VERSION &&
        entry.datasetVersion === this.descriptor.addonVersion &&
        entry.resourceId === id &&
        [1, 2, 3, 4].includes(entry.kind) &&
        [0, 1].includes(entry.encoding)
      ) {
        this.rememberResource(id, {
          kind: entry.kind,
    ...
    for (let start = 0; start < missing.length; start += 256) {
      const response = await this.request(
        `/v1/scenery/${this.descriptor.addonVersion}/resources/batch`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ResourceIds: missing.slice(start, start + 256),
          }),
          signal,
        },
      );
      if (!response.ok)
        throw new Error(`Scenery resource request failed (${response.status})`);
      ...
      if (u32() !== 0x42524341 || u16() !== 7)
        throw new Error("Invalid scenery resource batch");
      const cacheEntries: [string, CachedResource][] = [];
      for (let i = 0, count = u16(); i < count; i++) {
        const id = u32();
        const kind = view.getUint8(offset++);
        const encoding = view.getUint8(offset++);
        if (u16() !== 0) throw new Error("Invalid scenery resource");
        const encoded = u32();
        u32();
        const bytes = buffer.slice(offset, offset + encoded);
        offset += encoded;
        this.rememberResource(id, { kind, encoding, bytes });
        cacheEntries.push([
          this.cacheKey(`resource:${id}`),
          this.cached(bytes, id, kind, encoding),
        ]);
      }
  ...
  async mesh(modelIndex: number): Promise<import("./acdatclient").Mesh> {
    const model = this.models[modelIndex];
    if (!model) throw new Error("Missing scenery model");
    await this.loadResources([
      model.meshResourceId,
      ...this.dependencies.slice(
        model.dependencyStart,
        model.dependencyStart + model.dependencyCount,
      ),
    ]);
    const resource = this.resource(model.meshResourceId)!;
    return this.profiler.measure("mesh decode", () =>
      this.processor.decodeMesh({
        id: model.meshResourceId,
  ...
  async texture(
    id: number,
  ): Promise<import("./datprocessorprotocol").ProcessedResourceTexture> {
    const resource = this.resource(id);
    if (!resource) throw new Error(`Missing scenery texture ${id}`);
    return this.profiler.measure("texture decode", () =>
  ...
  private async fetchChunks(
    ids: number[],
    signal: AbortSignal,
  ): Promise<Map<number, SceneryPlacement[]>> {
    ...
    const response = await this.request(
      `/v1/scenery/${this.descriptor.addonVersion}/chunks/batch`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (!response.ok)
      throw new Error(
        `Scenery chunk batch request failed (${response.status})`,
      );
    ...
    if (u32() !== 0x42534341 || u16() !== 1)
      throw new Error("Invalid scenery chunk batch");
    ...
    for (let i = 0, count = u16(); i < count; i++) {
      ...
      if (
        ...
        throw new Error("Invalid scenery chunk batch entry");
      const bytes = view.buffer.slice(
        view.byteOffset + offset,
        view.byteOffset + offset + length,
      );
      offset += length;
      result.set(id, this.parseChunk(id, bytes));
      received.add(id);
      cacheEntries.push([
        this.cacheKey(`chunk:${id >>> 0}`),
        this.cached(bytes, id),
      ]);
    }
    if (
      offset !== view.byteLength ||
      received.size !== missing.length ||
      result.size !== ids.length
    )
      throw new Error("Invalid scenery chunk batch");
    void this.cache.setMany(cacheEntries).catch(() => undefined);
    return result;
  ...
  private parseChunk(id: number, bytes: ArrayBuffer): SceneryPlacement[] {
    ...
    if (bytes.byteLength % 36 !== 0)
      throw new Error("Invalid scenery chunk payload");
    for (let offset = 0; offset < view.byteLength; offset += 36)
      result.push({
        modelId: view.getUint32(offset, true),
        origin: [
        ...
        scale: view.getFloat32(offset + 32, true),
      });
    if (result.length !== entry.placementCount)
      throw new Error("Invalid scenery chunk placement count");
    return result;
  }

  private async readCached(ids: number[]): Promise<Map<number, ArrayBuffer>> {
    const keys = ids.map((id) => this.cacheKey(`chunk:${id >>> 0}`));
  ...
  private cached(
    ...
    return {
      formatVersion: SCENERY_FORMAT_VERSION,
      datasetVersion: this.descriptor.addonVersion,
      resourceId,
      kind,
      encoding,
      bytes,
  ...
  private cacheKey(name: string): string {
    return `scenery:${SCENERY_FORMAT_VERSION}:${this.descriptor?.addonVersion ?? "unknown"}:${name}`;
  }
  ...
  private rememberResource(
    id: number,
    resource: { kind: number; encoding: number; bytes: ArrayBuffer },
  ): void {
    ...
    while (
      this.resourceBytes > MAX_SCENERY_RESOURCE_BYTES &&

...
function GetLandblockFromCoordinates(EW: number, NS: number) {
  const lbX = Math.floor(
    ((EW + Math.abs(minCoord)) / (maxCoord - minCoord)) * LAND_BLOCK_SIDE,
  );
  const lbY = Math.floor(
    ((NS + Math.abs(minCoord)) / (maxCoord - minCoord)) * LAND_BLOCK_SIDE,
  );
...
export default class Coordinates {
  readonly CoordinateRegex: RegExp =
    /(?<NSval>[0-9]{1,3}(?:\\.[0-9]{1,3})?)(?<NSchr>(?:[ns]))(?:[,\\s]+)?(?<EWval>[0-9]{1,3}(?:\\.[0-9]{1,3})?)(?<EWchr>(?:[ew]))?(,?\\s*(?<Zval>\\-?\\d+.?\\d+)z)?/i;
  ...
  get NS() {
    return LandblockToNS(this.LandCell, this.LocalY);
  }
  get EW() {
    return LandblockToEW(this.LandCell, this.LocalX);
  }
  ...
  #CalculateOutdoorLandcell() {
    this.LandCell =
      (this.LandCell |
        (Math.floor(this.LocalX / TERRAIN_CELL_SIZE) *
          TERRAIN_CELLS_PER_LAND_BLOCK +
          Math.ceil(this.LocalY / TERRAIN_CELL_SIZE))) >>>
      0;
  }
...
export function worldToMapCoordinates(position: {
  x: number;
  y: number;
  z: number;
}): Coordinates {
...
export function mapCoordinatesToWorld(coords: Coordinates): {
  x: number;
  y: number;
  z: number;
} {

...
export function uploadResourceTexture(
  ...
  try {
    ...
    if (surface.bitmap) {
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, surface.bitmap)
    } else {
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, surface.width, surface.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, surface.pixels ?? null)
    }

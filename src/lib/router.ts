...
import Coordinates, { mapCoordinatesToWorld, worldToMapCoordinates } from './coordinates'
...
function makeRoute(route: CameraRoute) {
  ...
  return `3d,${mapPosition.join(',')},${formatAngle(route.yaw!)},${formatAngle(route.pitch!)},` +
...
function parseMapCoordinate(value: string, positive: string, negative: string) {
  const match = value.match(new RegExp(`^(\\d+(?:\\.\\d+)?)([${positive}${negative}])$`, 'i'))
...
export function parseRoute(route: string): CameraRoute | undefined {
  ...
  if (parts[0] === "2d" && parts.length === 5) {
    ...
    if (northSouth === undefined || eastWest === undefined || z === undefined || !Number.isFinite(zoom)) return undefined
    const position = mapCoordinatesToWorld(Coordinates.FromCoordinates(northSouth, eastWest, z))
    return {
      mode: "2d",
  ...
  if (parts[0] === "3d" && parts.length === 8) {
    ...
    if ([northSouth, eastWest, z, yaw, pitch, roll].some(value => value === undefined) || !Number.isFinite(fov)) return undefined
    const position = mapCoordinatesToWorld(Coordinates.FromCoordinates(northSouth!, eastWest!, z!))
    return {
      mode: "3d",
      position,

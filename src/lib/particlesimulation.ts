...
export class ParticleSimulation {
  ...
  update(
    ...
    this.timeRunning += dt;
    const withinTime =
      p.totalSeconds === 0 || this.timeRunning < p.totalSeconds;
    const withinCount =
      p.totalParticles === 0 || this.totalEmitted < p.totalParticles;
    ...
    if (canEmit) {
      if (this.totalEmitted === 0 && p.initialParticles > 0) {
        for (
          let i = 0;
          i < p.initialParticles && this.particles.length < p.maxParticles;
          i++
        )
      ...
      if (p.emitterType === 1 || p.emitterType === 0) {
        ...
        const intervalElapsed =
          interval <= 0.001 || this.emissionTimer > interval;
        if (
          intervalElapsed &&
          this.particles.length < Math.max(1, p.maxParticles)
        ) {
  ...
  private emit(
    ...
    const maxLifetime = Math.max(
      0.001,
      p.lifespan + (this.randomValue() * 2 - 1) * p.lifespanRandom,
    );
    const frameRotation = this.mulQuat(placementRotation, p.parentOrientation);
    const offset = this.randomOffset();
    const worldOffset = this.rotate(
      frameRotation,
      this.scaleVector(this.add(p.offset, offset), placementScale),
    );
    const parentOrigin = this.add(
      placementOrigin,
      this.rotate(
        placementRotation,
        this.scaleVector(p.parentOrigin, placementScale),
      ),
    );
    ...
    const particle: ParticleState = {
      ...
      lifetime: 0,
      maxLifetime,
      startScale: this.clamp(
        p.startScale + (this.randomValue() * 2 - 1) * p.scaleRandom,
        0.1,
        10,
      ),
      finalScale: this.clamp(
        p.finalScale + (this.randomValue() * 2 - 1) * p.scaleRandom,
        0.1,
        10,
      ),
      startTranslucency: this.clamp(
        p.startTranslucency +
          (this.randomValue() * 2 - 1) * p.translucencyRandom,
        0,
        1,
      ),
      finalTranslucency: this.clamp(
        p.finalTranslucency +
          (this.randomValue() * 2 - 1) * p.translucencyRandom,
        0,
        1,
  ...
  private instances(
    ...
    for (
      let index = 0;
      index < this.particles.length && index < maxInstances;
      index++
    ) {
      ...
      const opacity =
        1 -
        this.lerp(particle.startTranslucency, particle.finalTranslucency, life);
      ...
      result.push({
        ...
        opacity,
        dimensions: [
          Math.abs(p.dimensions[0] * placementScale[0]),
          Math.abs(p.dimensions[1] * placementScale[1]),
          Math.abs(p.dimensions[2] * placementScale[2]),
        ],
        planeSize: this.planeSize(p.dimensions, placementScale),
        centerOffset: [
          p.centerOffset[0] * placementScale[0],
          p.centerOffset[1] * placementScale[1],
          p.centerOffset[2] * placementScale[2],
        ],
  ...
  private calculatePosition(
    ...
    switch (this.descriptor.particleType) {
      ...
      case 6:
        return [
          (t * particle.worldB[0] + particle.worldC[0] * particle.worldA[0]) *
            t +
            baseX,
          (t * particle.worldB[1] + particle.worldC[1] * particle.worldA[0]) *
            t +
            baseY,
          (t * particle.worldB[2] +
            particle.worldC[2] * particle.worldA[0] +
            particle.worldA[2]) *
            t +
            baseZ,
        ];
  ...
  private randomize(slot: number): void {
    let x = (this.descriptor.seed ^ Math.imul(slot + 1, 668265263)) >>> 0;
    for (let i = 0; i < this.random.length; i++) {
      x = (Math.imul(x, 1664525) + 1013904223) >>> 0;
      this.random[i] = (x >>> 8) / 16777216;
    }
    this.randomCursor = 0;
  }
  private randomValue(): number {
    return this.random[this.randomCursor++];
  }
  private randomVector(
    ...
    const m = min + (max - min) * this.randomValue();
    return this.mul(v, m);
  }
  private randomOffset(): [number, number, number] {
    const v: [number, number, number] = [
      this.randomValue() * 2 - 1,
      this.randomValue() * 2 - 1,
      this.randomValue() * 2 - 1,
    ];
    const d = this.descriptor.offsetDirection;
    const dot = v[0] * d[0] + v[1] * d[1] + v[2] * d[2];
    return this.mul(
      this.normalize([v[0] - d[0] * dot, v[1] - d[1] * dot, v[2] - d[2] * dot]),
      this.descriptor.minOffset +
        (this.descriptor.maxOffset - this.descriptor.minOffset) *
          this.randomValue(),
    );
  }
  ...
  private planeSize(
    ...
    if (y > x && y > z)
      return x > z
        ? [Math.abs(x * scale[0]), Math.abs(y * scale[1])]
        : [Math.abs(y * scale[1]), Math.abs(z * scale[2])];
    if (x > y && x > z)
      return z > y
        ? [Math.abs(x * scale[0]), Math.abs(z * scale[2])]
        : [Math.abs(x * scale[0]), Math.abs(y * scale[1])];
    return x > y
      ? [Math.abs(x * scale[0]), Math.abs(z * scale[2])]
      : [Math.abs(y * scale[1]), Math.abs(z * scale[2])];

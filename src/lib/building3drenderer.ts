...
export class Building3DRenderer {
  ...
  constructor(private gl: WebGL2RenderingContext) {
    ...
    this.uniforms = this.program ? {
      ...
      buildingTexture: gl.getUniformLocation(this.program, 'buildingTexture'),
      placementOrigin: gl.getUniformLocation(this.program, 'placementOrigin'),
      placementRotation: gl.getUniformLocation(this.program, 'placementRotation'),
  ...
  isBuildingLoaded(landblockX: number, landblockY: number, mapX: number, mapY: number): boolean {
    ...
    for (const landblock of this.landblocks.values()) {
      ...
      for (const building of landblock.buildings) {
        ...
        const buildingMapY = MAP_SIZE - (landblock.y * LAND_BLOCK_SIZE + building.origin[1])
        const mesh = this.meshes.get(building.modelIndex)
        if (Math.abs(buildingMapX - mapX) <= matchDistance && Math.abs(buildingMapY - mapY) <= matchDistance &&
          !!mesh) {
          return true
        }
      }
  ...
  render(camera: BaseCamera, mode: CameraMode, enabled: boolean, minimumZoom: number): void {
    if (!enabled || !this.program || !this.uniforms || !this.vao || !this.hasBuildings ||
      !this.isCloseEnough(camera, mode, minimumZoom)) return
    const visible = mode === CameraMode.Camera2D
    ...
    gl.uniform1i(this.uniforms.cameraMode, mode === CameraMode.Camera2D ? 0 : 1)
    ...
    try {
      for (const [x, y] of populated) {
        ...
        if (!landblock) continue
        for (const building of landblock.buildings) this.drawBuilding(landblock, building)
      }
    } finally {
  ...
  private drawBuilding(landblock: LoadedLandblock, building: IndexedPlacement): void {
    const mesh = this.meshes.get(building.modelIndex)
    if (mesh === undefined) {
    ...
    if (!mesh) {
      this.reportBuildingFailure(landblock, building, 'model could not be loaded')
      return
    }
    if (mesh.batches.length === 0) {
      this.reportBuildingFailure(landblock, building, 'model has no drawable mesh batches')
      return
    }
    const gl = this.gl
    const uniforms = this.uniforms!
    gl.uniform3f(uniforms.placementOrigin,
    ...
    for (const batch of mesh.batches) {
      if (batch.count === 0) {
        this.reportBuildingFailure(landblock, building, 'model contains an empty mesh batch')
        continue
      }
      ...
      if (batch.material.translucent) {
        gl.enable(gl.BLEND)
        gl.blendFunc(gl.SRC_ALPHA, batch.material.additive ? gl.ONE : gl.ONE_MINUS_SRC_ALPHA)
        gl.depthMask(false)
      } else {
        gl.disable(gl.BLEND)
        gl.depthMask(true)
  ...
  private requestLandblocks(visible: [number, number][]): void {
    ...
    this.dats.loadVisible(visible).then(() => {
      ...
      for (const [x, y] of pending) this.pendingLandblocks.delete(`${x},${y}`)
  ...
  private requestMesh(modelIndex: number): void {
    ...
    this.dats.mesh(modelIndex).then(mesh => {
      this.meshes.set(modelIndex, this.uploadMesh(mesh.batches))
    }).catch(error => {
      console.error(`Unable to load ACTerrain building model index ${modelIndex}`, error)
  ...
  private uploadMesh(source: { materialResourceId: number, vertices?: Float32Array }[]): GpuMesh {
    const batches: GpuBatch[] = source.filter(item => item.vertices).map(item => {
      const vertices = item.vertices!
      const buffer = this.gl.createBuffer()!
      this.gl.bindBuffer(this.gl.ARRAY_BUFFER, buffer)
      this.gl.bufferData(this.gl.ARRAY_BUFFER, vertices, this.gl.STATIC_DRAW)
      const batch: GpuBatch = { buffer, count: vertices.length / 8, materialResourceId: item.materialResourceId }
      this.dats.material(item.materialResourceId).then(material => batch.material = material).catch(error => {
  ...
  private reportBuildingFailure(landblock: LoadedLandblock, building: IndexedPlacement, reason: string): void {
  ...
  private visible2D(camera: Camera2D): [number, number][] {
    ...
    const maxX = Math.min(MAX_LAND_BLOCK_INDEX, Math.floor(Math.max(topLeft.x, bottomRight.x) / LAND_BLOCK_SIZE))
    const minY = Math.max(0, Math.floor((MAP_SIZE - Math.max(topLeft.y, bottomRight.y)) / LAND_BLOCK_SIZE))
    const maxY = Math.min(MAX_LAND_BLOCK_INDEX,
      Math.floor((MAP_SIZE - Math.min(topLeft.y, bottomRight.y)) / LAND_BLOCK_SIZE))
  ...
  private visible3D(camera: CameraFlying): [number, number][] {
    ...
    for (const position of positions) {
      ...
      for (const landblock of this.range(
        Math.max(0, centerX - this.loadDistance), Math.min(MAX_LAND_BLOCK_INDEX, centerX + this.loadDistance),
        Math.max(0, centerY - this.loadDistance), Math.min(MAX_LAND_BLOCK_INDEX, centerY + this.loadDistance))) {
  ...
  private range(minX: number, maxX: number, minY: number, maxY: number): [number, number][] {
    const result: [number, number][] = []
    for (let y = minY; y <= maxY; y++) for (let x = minX; x <= maxX; x++) result.push([x, y])
    return result
  }
}

...
const requestedMaxBytes = Number(new URLSearchParams(location.search).get('maxBytes'))
const MAX_CACHE_BYTES = Number.isFinite(requestedMaxBytes) && requestedMaxBytes > 0 ? requestedMaxBytes : 384 * 1024 * 1024
...
function diagnostics(): CacheDiagnostics {
  return { enabled, usageBytes: estimatedUsage, quotaBytes: estimatedQuota, cacheBytes: cacheBytes() }
}
...
function readExact(handle: SyncAccessHandle, length: number, at: number): Uint8Array {
  const result = new Uint8Array(length)
  if (handle.read(result, { at }) !== length) throw new Error('Unexpected end of OPFS cache file')
  return result
}

function writeExact(handle: SyncAccessHandle, bytes: Uint8Array, at: number): void {
  if (handle.write(bytes, { at }) !== bytes.byteLength) throw new Error('Incomplete OPFS cache write')
}
...
function validatePack(handle: SyncAccessHandle, namespace: CacheNamespace): number {
  const size = handle.getSize()
  if (size < PACK_HEADER_BYTES) throw new Error('Missing OPFS pack header')
  const bytes = readExact(handle, PACK_HEADER_BYTES, 0)
  const view = new DataView(bytes.buffer)
  if (view.getUint32(0, true) !== PACK_MAGIC || view.getUint16(4, true) !== FORMAT_VERSION ||
    view.getUint8(6) !== namespaceCode(namespace) || view.getUint16(8, true) !== PACK_HEADER_BYTES ||
...
function serializeIndex(state: NamespaceState): Uint8Array {
  ...
  const strings = [...state.entries.values()].map(entry => {
    ...
    if (key.byteLength > 0xffff || dataset.byteLength > 0xffff) throw new Error('OPFS cache index string is too long')
...
function parseIndex(handle: SyncAccessHandle, packHandle: SyncAccessHandle, namespace: CacheNamespace): Omit<NamespaceState, 'slot' | 'packHandle' | 'indexHandle'> {
  const size = handle.getSize()
  ...
  if (headerView.getUint32(0, true) !== INDEX_MAGIC || headerView.getUint16(4, true) !== FORMAT_VERSION ||
    headerView.getUint8(6) !== namespaceCode(namespace) || size !== INDEX_HEADER_BYTES + payloadLength) {
    throw new Error('Invalid OPFS index header')
  }
  const packSize = validatePack(packHandle, namespace)
  if (headerView.getFloat64(16, true) !== packSize) throw new Error('OPFS index and pack lengths do not match')
  const payload = readExact(handle, payloadLength, INDEX_HEADER_BYTES)
  if (headerView.getUint32(28, true) !== crc32(payload)) throw new Error('Invalid OPFS index checksum')
  const bytes = new Uint8Array(INDEX_HEADER_BYTES + payloadLength)
  bytes.set(header)
  ...
  for (let index = 0; index < headerView.getUint32(12, true); index++) {
    if (offset + 44 > bytes.byteLength) throw new Error('Truncated OPFS index entry')
    ...
    offset += 44
    if (offset + keyLength + datasetLength > bytes.byteLength || entry.offset < PACK_HEADER_BYTES + RECORD_HEADER_BYTES ||
      entry.offset + entry.length > packSize) throw new Error('Invalid OPFS index entry')
    entry.key = decoder.decode(bytes.subarray(offset, offset + keyLength))
    offset += keyLength
    entry.datasetVersion = decoder.decode(bytes.subarray(offset, offset + datasetLength))
  ...
  if (offset !== bytes.byteLength) throw new Error('OPFS index has trailing bytes')
  return { namespace, generation: headerView.getUint32(8, true), entries, packSize, activeBytes }
}

async function openHandle(name: string): Promise<SyncAccessHandle> {
  const file = await root!.getFileHandle(name, { create: true }) as SyncFileHandle
  return file.createSyncAccessHandle()
}

function names(namespace: CacheNamespace, slot: number): { pack: string; index: string } {
  return { pack: `${namespace}-${slot}.pack`, index: `${namespace}-${slot}.index` }
}

async function openSlot(namespace: CacheNamespace, slot: number): Promise<NamespaceState | null> {
  ...
  if (!packHandle || !indexHandle) throw new Error('Unable to open OPFS cache files')
  try {
...
async function createSlot(namespace: CacheNamespace, slot: number, generation: number): Promise<NamespaceState> {
  const files = names(namespace, slot)
  let packHandle: SyncAccessHandle | null = null
  let indexHandle: SyncAccessHandle | null = null
  try {
  ...
  if (!packHandle || !indexHandle) throw new Error('Unable to create OPFS cache files')
  packHandle.truncate(0)
  const header = packHeader(namespace)
  writeExact(packHandle, header, 0)
  packHandle.flush()
  const state: NamespaceState = { namespace, slot, generation, packHandle, indexHandle, entries: new Map(), packSize: header.byteLength, activeBytes: 0 }
  persistIndex(state)
  return state
}

async function removeSlot(namespace: CacheNamespace, slot: number): Promise<void> {
  const files = names(namespace, slot)
  await root!.removeEntry(files.pack).catch(() => undefined)
  await root!.removeEntry(files.index).catch(() => undefined)
}

async function openNamespace(namespace: CacheNamespace): Promise<NamespaceState> {
...
function validateRecord(state: NamespaceState, entry: IndexEntry): Uint8Array {
  const header = readExact(state.packHandle, RECORD_HEADER_BYTES, entry.offset - RECORD_HEADER_BYTES)
  const view = new DataView(header.buffer)
  if (view.getUint32(0, true) !== RECORD_MAGIC || view.getUint16(4, true) !== FORMAT_VERSION ||
    view.getUint16(6, true) !== RECORD_HEADER_BYTES || view.getUint32(8, true) !== entry.length ||
    view.getUint32(12, true) !== entry.checksum) throw new Error('Invalid OPFS cache record')
  const bytes = readExact(state.packHandle, entry.length, entry.offset)
  if (crc32(bytes) !== entry.checksum) throw new Error('Invalid OPFS cache payload checksum')
  return bytes
}
...
function evictToTarget(): void {
  const entries = [...states.values()].flatMap(state => [...state.entries.values()].map(entry => ({ state, entry })))
...
async function writeBytes(handle: SyncAccessHandle, bytes: Uint8Array, at: number): Promise<void> {
  for (let offset = 0; offset < bytes.byteLength; offset += WRITE_CHUNK_BYTES) {
    writeExact(handle, bytes.subarray(offset, Math.min(bytes.byteLength, offset + WRITE_CHUNK_BYTES)), at + offset)
    if (offset + WRITE_CHUNK_BYTES < bytes.byteLength) await new Promise(resolve => setTimeout(resolve, 0))
  }
}
...
async function flushWrites(): Promise<void> {
  ...
  let quotaBudget = estimatedQuota > 0 ? estimatedQuota - estimatedUsage - QUOTA_RESERVE_BYTES : Number.POSITIVE_INFINITY
  let budget = WRITE_BATCH_BYTES
  const batches = new Map<CacheNamespace, [string, CachedResource][]>([['terrain', []], ['scenery', []]])
  ...
  for (const namespace of ['terrain', 'scenery'] as const) {
    ...
    if (readQueue.length > 0) {
      for (const [key, value] of batch) pendingWrites.get(namespace)!.set(key, value)
      batch.length = 0
      flushDue = true
      continue
    }
    for (let batchIndex = 0; batchIndex < batch.length; batchIndex++) {
      ...
      if (batchIndex + 1 < batch.length) {
        ...
        if (readQueue.length > 0) {
          for (const [remainingKey, remainingValue] of batch.slice(batchIndex + 1)) pendingWrites.get(namespace)!.set(remainingKey, remainingValue)
  ...
  evictIfNeeded()
  for (const namespace of ['terrain', 'scenery'] as const) {
    if (batches.get(namespace)!.length > 0) persistIndex(states.get(namespace)!)
  }
  timing('OPFS flush', performance.now() - started)
  if ([...pendingWrites.values()].some(writes => writes.size > 0)) {
    flushDue = true
  }
  if ([...states.values()].some(state => state.packSize > state.activeBytes * COMPACTION_PHYSICAL_FACTOR + PACK_HEADER_BYTES)) compactionDue = true
}
  ...
  try {
    if (task.operation === 'clear') {
      ...
      for (const entry of [...state.entries.values()]) {
        if (entry.formatVersion === task.formatVersion && entry.datasetVersion === task.datasetVersion) continue
    ...
    post({ type: 'error', id: task.id, message: error instanceof Error ? error.message : String(error) })
  }
}
...
async function clearNamespace(namespace: CacheNamespace): Promise<void> {
  ...
  await removeSlot(namespace, 0)
  await removeSlot(namespace, 1)
  states.set(namespace, await createSlot(namespace, 0, (state?.generation ?? 0) + 1))
  post({ type: 'diagnostics', diagnostics: diagnostics() })
}
...
async function compactNamespace(namespace: CacheNamespace): Promise<void> {
  ...
  if (source.packSize <= source.activeBytes * COMPACTION_PHYSICAL_FACTOR + PACK_HEADER_BYTES) return
  const started = performance.now()
...
async function compact(): Promise<void> {
  ...
  if ([...pendingWrites.values()].some(writes => writes.size > 0)) flushDue = true
}
...
async function pump(): Promise<void> {
  ...
  try {
    ...
    processing = false
    if (enabled && (readQueue.length > 0 || controlQueue.length > 0 || flushDue || indexPersistenceDue || compactionDue)) void pump()
  }
}
...
function disable(reason: string): void {
  ...
  for (const state of states.values()) {
    try { state.packHandle.close() } catch { /* already closed */ }
    try { state.indexHandle.close() } catch { /* already closed */ }
  }
  states.clear()
  for (const task of readQueue.splice(0)) post({ type: 'result', id: task.id, values: task.keys.map(() => null) })
  for (const task of controlQueue.splice(0)) post({ type: 'result', id: task.id })
...
scope.onmessage = event => {
  ...
  if (request.operation === 'shutdown') {
    enabled = false
    for (const state of states.values()) {
      try { persistIndex(state) } catch { /* Cache is disposable. */ }
      try { state.packHandle.close() } catch { /* already closed */ }
      try { state.indexHandle.close() } catch { /* already closed */ }
    }
    states.clear()
  ...
  if (!enabled) {
    if ('id' in request) {
      if (request.operation === 'getMany') post({ type: 'result', id: request.id, values: request.keys.map(() => null) })

...
import { intersectsRectangle } from "./objectvisibility";
import {
  LAND_BLOCK_SIZE,
  MAP_SIZE,
...
interface CachedMaterial {
  promise: Promise<Omit<Batch, "vertex" | "index" | "count">>;
  references: number;
}
export interface SceneryDiagnostics {
...
export class SceneryRenderer {
  ...
  constructor(
    ...
    this.uniforms = {
      ...
      texture: gl.getUniformLocation(this.program, "buildingTexture"),
      diffuse: gl.getUniformLocation(this.program, "diffuseAmount"),
      luminosity: gl.getUniformLocation(this.program, "luminosity"),
      opacity: gl.getUniformLocation(this.program, "opacity"),
    };
  }

  get frameDiagnostics(): SceneryDiagnostics {
  ...
  clear(): void {
    ...
    this.pending.clear();
    this.meshQueue = [];
    this.cachedGpuBytes = 0;
    this.instanceData.clear();
    this.client.clear();
    this.diagnostics = this.empty();
  }
  ...
  render(
    ...
    if (mode === CameraMode.Camera2D && (camera as Camera2D).Zoom < minimumZoom)
      return;
    const ids: number[] = [];
    const ranges =
    ...
    for (let y = ranges.minY; y <= ranges.maxY; y++)
      for (let x = ranges.minX; x <= ranges.maxX; x++) {
        const id = ((x << 24) | (y << 16) | 0xfffe) >>> 0;
        const entry = this.client.chunksById.get(id);
        if (entry && this.chunkVisible(entry, id, camera, mode)) ids.push(id);
      }
    const changed =
      ids.length !== this.visibleIds.length ||
      ids.some((id, index) => id !== this.visibleIds[index]);
    this.visibleIds = ids;
    this.diagnostics.visibleChunks = ids.length;
    this.diagnostics.loadedChunks = this.client.loadedChunkIds.length;
    if (changed) void this.client.loadChunks(ids);
    const models = new Set<number>();
    const grouped = new Map<number, number[]>();
    for (const id of ids)
      for (const placement of this.client.placementsForChunk(id)) {
        if (this.diagnostics.instances >= 100000) break;
        const modelIndex = this.client.modelIndex(placement.modelId);
        const model =
          modelIndex === undefined
            ? undefined
            : this.client.modelsByIndex[modelIndex];
        if (!model || modelIndex === undefined) continue;
        models.add(modelIndex);
        const list = grouped.get(modelIndex) ?? [];
        list.push(...this.transformInstance(placement, id));
        grouped.set(modelIndex, list);
        this.diagnostics.visiblePlacements++;
      }
    this.diagnostics.visibleModels = models.size;
    this.diagnostics.instances = this.diagnostics.visiblePlacements;
    this.scheduleMeshes(models);
    this.draw(camera, grouped);
    this.trimMeshes(models);
    this.diagnostics.gpuBytes = this.cachedGpuBytes;
    this.diagnostics.pendingRequests =
      this.client.pendingRequestCount + this.pending.size;
  }

  private visible2D(camera: Camera2D): {
    minX: number;
    maxX: number;
    minY: number;
    maxY: number;
  ...
  private draw(camera: BaseCamera, grouped: Map<number, number[]>): void {
    ...
    for (const [modelIndex, values] of grouped) {
      ...
      for (const batch of mesh.batches) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, this.instanceBuffer);
        gl.vertexAttribPointer(3, 3, gl.FLOAT, false, 40, 0);
        gl.vertexAttribPointer(4, 4, gl.FLOAT, false, 40, 12);
        ...
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, batch.index);
        gl.bindTexture(gl.TEXTURE_2D, batch.texture);
        gl.uniform1f(this.uniforms.diffuse, batch.diffuse);
        gl.uniform1f(this.uniforms.luminosity, batch.luminosity);
        gl.uniform1f(this.uniforms.opacity, batch.opacity);
        if (batch.translucent) {
          gl.enable(gl.BLEND);
          gl.blendFunc(
            gl.SRC_ALPHA,
            batch.additive ? gl.ONE : gl.ONE_MINUS_SRC_ALPHA,
          );
          gl.depthMask(false);
        } else {
          gl.disable(gl.BLEND);
  ...
  private async upload(
    modelIndex: number,
    source: import("./acdatclient").Mesh,
  ): Promise<void> {
    const batches: Batch[] = [];
    let gpuBytes = 0;
  ...
  private acquireMaterial(
    id: number,
  ): Promise<Omit<Batch, "vertex" | "index" | "count">> {
  ...
  private async loadMaterial(
    ...
    const resource = this.client.resource(id);
    if (!resource) throw new Error(`Missing scenery material ${id}`);
    const bytes =
      resource.encoding === 0
        ? resource.bytes
        : await new Response(
    ...
    if ((flags & 1) !== 0) {
      try {
        const textureId = view.getUint32(16, true);
        texture = uploadResourceTexture(
          this.gl,
          await this.client.texture(textureId),
        );
      } catch (error) {
        console.warn(`Scenery texture for material ${id} skipped`, error);
        texture = this.solidTexture(new Uint8Array([160, 160, 160, 255]));
      }
    } else {
      texture = this.solidTexture(new Uint8Array(bytes.slice(16, 20)));
    }
    return {
      material: id,
      texture,
      ...
      additive: (flags & 4) !== 0,
      diffuse: view.getFloat32(8, true),
      luminosity: view.getFloat32(4, true),
      opacity: view.getFloat32(12, true),
    };
  }
  ...
  private transformInstance(p: SceneryPlacement, id: number): number[] {
    const x = (id >>> 24) & 255;
    const y = (id >>> 16) & 255;
    return [
      x * LAND_BLOCK_SIZE + p.origin[0],
      y * LAND_BLOCK_SIZE + p.origin[1],
      p.origin[2],
      p.rotation[0],
      p.rotation[1],
      p.rotation[2],
  ...
  private chunkVisible(
    ...
    const blockX = (id >>> 24) & 255;
    const blockY = (id >>> 16) & 255;
    const x = blockX * LAND_BLOCK_SIZE;
    const y = blockY * LAND_BLOCK_SIZE;
    const bounds = {
      minimum: [
        x + entry.bounds.minimum[0],
        MAP_SIZE - (y + entry.bounds.maximum[1]),
        entry.bounds.minimum[2],
      ] as [number, number, number],
      maximum: [
        x + entry.bounds.maximum[0],
        MAP_SIZE - (y + entry.bounds.minimum[1]),
        entry.bounds.maximum[2],
      ] as [number, number, number],
    };
  ...
  private pumpMeshQueue(): void {
    while (
      this.activeMeshLoads < MAX_MESH_LOADS_IN_FLIGHT &&
      ...
      void this.client
        .mesh(modelIndex)
        .then((mesh) =>
          this.profiler.measure("model upload", () =>
            this.upload(modelIndex, mesh),
          ),
        )
        .catch((error) => {
  ...
  private trimMeshes(visible: Set<number>): void {
    while (
      this.cachedGpuBytes > this.maxGpuBytes ||
      this.meshes.size > MAX_CACHED_MESHES
    ) {
  ...
  private empty(): SceneryDiagnostics {
    return {
      visibleChunks: 0,
      loadedChunks: 0,
      visiblePlacements: 0,

...
export const TERRAIN_DATA_SIDE = LAND_BLOCK_SIDE * TERRAIN_CELLS_PER_LAND_BLOCK + 1
export const MAX_TERRAIN_HEIGHT = terrainHeightTable[terrainHeightTable.length - 1]
export const OBJECT_Z_BIAS = 0.03
...
export function landBlockId(x: number, y: number): number {
  return ((clampLandBlockIndex(x) << 24) | (clampLandBlockIndex(y) << 16)) >>> 0
}

...
export function parseLocationTargets(query: string): LocationTarget[] {
  const map = query.match(/^(\d+(?:\.\d+)?)\s*([NS])\s*[,\s]\s*(\d+(?:\.\d+)?)\s*([EW])$/i);
  if (map) {
  ...
  const location = query.match(/^(?:0x)?([0-9a-f]{8})\s*\[([^\]]+)\](?:\s+(.+))?$/i);
  if (location) {
    ...
    if (coordinates.length !== 3 || !coordinates.every(Number.isFinite) ||
        (quaternion && (quaternion.length !== 4 || !quaternion.every(Number.isFinite) || Math.hypot(...quaternion) === 0))) {
      return [];
    }
  ...
  return [
    { text: `Open dungeon ${hexId(landblock, 4)}`, landblock,
      cellId: cell >= 0x100 && cell < 0xfffe ? id : undefined },
    { text: `View landscape ${hexId(landblock, 4)}`, zoom: 1,
      position: { x: (landblock >>> 8) * LAND_BLOCK_SIZE + LAND_BLOCK_SIZE / 2,
        y: MAP_SIZE - ((landblock & 0xff) * LAND_BLOCK_SIZE + LAND_BLOCK_SIZE / 2), z: 0 } },
  ];
}

...
vec4 CombineOverlays(BaseData base, TerrainData terrains) {
  ...
  result = maskBlend3(overlay0, overlay1, overlay2, 1., 1., 1.);
...
ivec4 rot2(int rot, ivec4 t) {
  if (rot == 0) {
    return ivec4(t[2], t[3], t[0], t[1]);
  }
  else if (rot == 1) {
    return t;
    return ivec4(t[1], t[2], t[3], t[0]);
  }
  else if (rot == 2) {
    return ivec4(t[1], t[2], t[3], t[0]);
    return t;
    return ivec4(t[3], t[0], t[1], t[2]);
  }
  else if (rot == 3) {
    return ivec4(t[3], t[0], t[1], t[2]);
    return t;
...
vec4 getSplattedTerrainColor(vec3 pos) {
  ...
  vec4 c1 = texture(terrainAtlas, vec3(cPos.xy, t1));
  ...
  ivec4 rotT0 = rot2(tAlpha0.x, ivec4(t1, t2, t3, t4));
  ivec4 rotT1 = rot2(tAlpha1.x, ivec4(t1, t2, t3, t4));
  ivec4 rotT2 = rot2(tAlpha2.x, ivec4(t1, t2, t3, t4));

  int i1 = (tAlpha0.x == 0 ? t4 : (tAlpha0.x == 1 ? t3 : (tAlpha0.x == 2 ? t3 : t1)));
  int i2 = (tAlpha0.x == 0 ? t4 : (tAlpha0.x == 1 ? t1 : (tAlpha0.x == 2 ? t2 : t3)));
  int i3 = (tAlpha0.x == 0 ? t4 : (tAlpha0.x == 1 ? t4 : (tAlpha0.x == 2 ? t3 : t1)));
  int i4 = (tAlpha0.x == 0 ? t4 : (tAlpha0.x == 1 ? t2 : (tAlpha0.x == 2 ? t3 : t1)));

  BaseData base = BaseData(vec4(wpos.xyz, 1), vec3(cPos.xy, t1));
  TerrainData terrains = TerrainData(
    vec4(getRot(tAlpha0.x, cPos.xy), rotT0[0], tAlpha0.y),
    vec4(getRot(tAlpha1.x, cPos.xy), rotT1[1], tAlpha1.y),
    vec4(getRot(tAlpha2.x, cPos.xy), rotT2[2], tAlpha2.y)
  );
...
void main() {
  ...
  int tCode = int(texelFetch(terrainData, ivec2(cPos + vec2(0, 1)), 0).g * 255.0);

    ...
    if (request.operation === "mesh") {
      ...
      for (const batch of mesh.batches) {
        if (batch.vertices && batch.indices)
          transfer.push(
            batch.vertices.buffer as ArrayBuffer,
            batch.indices.buffer as ArrayBuffer,
          );
      }
...
async function decodeMesh(resource: EncodedDatResource): Promise<Mesh> {
  ...
  for (let i = 0, count = u32(); i < count; i++) {
    ...
    if (batchKind === 0) {
      ...
      offset += indexBytes;
      batches.push({ materialResourceId, vertices, indices });
      ...
      for (let p = 0; p < count; p++) {
        ...
        const quat = () =>
          [f32(), f32(), f32(), f32()] as [number, number, number, number];
        const center = vector();
        const scale = f32();
        const opacity = f32();
        const dimensions = vector();
        const centerOffset = vector();
        const planeOrientation = quat();
        const rotation = quat();
        const billboard = view.getUint8(offset++) !== 0;
        offset += 3;
        particles.push({
          center,
          scale,
...
async function decodeTexture(
  ...
  const reservedFlags =
    view.getUint8(offset++) | view.getUint8(offset++) | view.getUint8(offset++);
  const length = view.getUint32(offset, true);
  offset += 4;
  if (reservedFlags !== 0 || offset + length + 4 > buffer.byteLength)
    throw new Error(`Invalid ACTerrain texture resource ${resource.id}`);
  const pixels = new Uint8Array(buffer.slice(offset, offset + length));
  offset += length;
  ...
  if (
    (flags & ~3) !== 0 ||
    reserved !== 0 ||
    ((flags & 2) !== 0) !== paletteCount > 0 ||
    offset + paletteLength !== buffer.byteLength
  ) {
    throw new Error(`Invalid ACTerrain texture resource ${resource.id}`);
  }
  ...
  const palette = paletteCount
    ? new Uint8Array(buffer.slice(offset, offset + paletteLength))
    : null;
...
function decodePixels(
  ...
  for (let i = 0; i < width * height; i++) {
    ...
    if (format === 0x14 || format === 0xf2 || format === 0xf3) {
      ...
      output[target + 2] =
        format === 0xf3 ? pixels[source + 2] : pixels[source];
      output[target + 3] = 255;
      ...
      decode16(
        format,
        pixels[i * 2] | (pixels[i * 2 + 1] << 8),
        output,
        target,
      );
    } else if (format === 0x29 || format === 0x65) {
      const index =
        format === 0x29 ? pixels[i] : pixels[i * 2] | (pixels[i * 2 + 1] << 8);
...
function decodeBc(
  ...
  for (let blockY = 0; blockY < Math.ceil(height / 4); blockY++) {
    for (
      let blockX = 0;
      blockX < Math.ceil(width / 4);
      blockX++, offset += blockSize
    ) {
      const alpha =
        kind === 1
          ? null
          : kind === 2
            ? bc2Alpha(source, offset)
            : bc3Alpha(source, offset);
...
function bc2Alpha(source: Uint8Array, offset: number): Uint8Array {
  ...
  for (let i = 0; i < 16; i++)
    alpha[i] =
      ((source[offset + Math.floor(i / 2)] >> ((i & 1) * 4)) & 15) * 17;
  return alpha;
}
...
function bc3Alpha(source: Uint8Array, offset: number): Uint8Array {
  ...
  if (table[0] > table[1]) {
    for (let i = 1; i <= 6; i++)
      table[i + 1] = Math.round(((7 - i) * table[0] + i * table[1]) / 7);
  } else {
    for (let i = 1; i <= 4; i++)
      table[i + 1] = Math.round(((5 - i) * table[0] + i * table[1]) / 5);
    table[6] = 0;
    table[7] = 255;
  }
  let bits = 0n;
  for (let i = 0; i < 6; i++)
    bits |= BigInt(source[offset + 2 + i]) << BigInt(i * 8);
  const alpha = new Uint8Array(16);
  for (let i = 0; i < 16; i++)
